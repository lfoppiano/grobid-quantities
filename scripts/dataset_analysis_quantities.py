...
def process_file(input):
    ...
    batch = 'batch-0'
    parent_directory = Path(input).parent.name
    if (parent_directory.startswith('batch')):
        batch = parent_directory
    ...
    for paragraph in children:
        for item in paragraph:
            paragraphText = ''
            document_statistics['sentences'] += 1
            sentence_entities = 0

            if type(item) == NavigableString:
                paragraphText += str(item)

            elif type(item) is Tag and item.name == 'measure' and 'type' in item.attrs and item.attrs['type'] in [
                'value', 'interval', 'range', 'list']:
            ...
            if sentence_entities > 0:
                document_statistics['positive_examples'] += 1
            else:
                document_statistics['negative_examples'] += 1

            document_statistics['tokens'] += len(tokenizeSimple(paragraphText))
            document_statistics['classes'] = len(set(entities_statistics.keys()))
            # document_statistics['sentences'] += len(text_to_sentences(paragraphText).split("\n"))
    ...
    assert "Number of total entities per document does not correspond to the sum.", total_entities == \
...
def aggregate(entities_statistics, threshold, skip=['tcValue', 'pressure'], only=None):
    """
...
def run_analysis(input):
    ...
    for document_statistics in documents_statistics:
        ...
        output_data['positive_examples'] += document_statistics['positive_examples']
        output_data['negative_examples'] += document_statistics['negative_examples']
...
if __name__ == "__main__":
    ...
    ## Data analysis
    columns = ['files', 'sentences', 'tokens', 'entities', 'uniq_entities', 'classes', 'positive_examples',
               'negative_examples']
