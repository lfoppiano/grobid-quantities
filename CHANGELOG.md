...
+ `Unit_Type` and `System_Type` are read from `lexicon/en/unit-vocabulary.json` instead of being Java enumerations, so a unit type is added in one place (#92)
...
+ 52 units filling gaps in the lexicon, including the first units for eleven declared-but-empty measurement types (#92)
+ `doc/lexicon.md`, documenting the lexicon, how to add a unit and a unit type, and an assessment of adopting the OM ontology (#92)
+ `QuantityLexiconLoadingTest` and `UnitUtilitiesVocabularyTest`, checking that the lexicon loads and resolves, not only that its JSON parses (#92)
...
+ `QuantityLexicon.derivationalMorphologyExpansion` no longer throws on a notation whose only separator is `·`, which made the whole lexicon fail to load (#92)
