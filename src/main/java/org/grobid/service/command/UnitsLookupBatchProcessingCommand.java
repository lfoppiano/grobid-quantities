...
public class UnitsLookupBatchProcessingCommand extends ConfiguredCommand<GrobidQuantitiesConfiguration> {
    private static final Logger LOGGER = LoggerFactory.getLogger(UnitsLookupBatchProcessingCommand.class);
    ...
    public UnitsLookupBatchProcessingCommand() {
        super("lookupUnits", "Lookup units files in batch");
    }
    ...
    protected void run(Bootstrap bootstrap, Namespace namespace, GrobidQuantitiesConfiguration configuration) throws Exception {
        ...
        new QuantitiesEngine().unitsLookupBatchProcess(inputDirectory, outputDirectory, isRecursive);
    }
}